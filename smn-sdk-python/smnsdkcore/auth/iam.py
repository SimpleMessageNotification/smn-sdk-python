...
class Auth():
    ...
    def _token_is_valid(self):
        ...
        logger.debug("(now_utc + 30 minutes) is %s, token expire time is %s.", expect_token_expire_time,
                     token_expire_time)
...
class AkskNova:
    ...
    NOTE:
    1. If you need to obtain temporary AKs ad SKs on ECSs, authorize the op_svc_ecs account in the IAM service.
    2. Temporary AKs and SKs expire an hour later.
    3. When using temporary AKs and SKs, add 'X-Security-Token':securitytoken in the message header.
    securitytoken is the value returned when a call is made to the API.
    '''

    def __init__(self, host='169.254.169.254', api_path='/openstack/latest/securitykey'):
        self.host = host
        self.api_path = api_path

    def get_temporary_aksk(self):
        '''
        :return:{"credential": {"access": "VC0057SP7GKPL...", "secret": "CFSIwrOIAhbXzKFhypmnY...",
        "securitytoken": "If7Xs9BLfNif6j3VBD9...", "expires_at": "2019-05-06T04:37:30.174000Z"}}
        '''
        resp = requests.request("GET", "http" + "://" + self.host + self.api_path)
        if resp.status_code < httplib.OK or resp.status_code >= httplib.MULTIPLE_CHOICES:
            raise SMNException(resp.status_code, resp.content)
        return resp.content
